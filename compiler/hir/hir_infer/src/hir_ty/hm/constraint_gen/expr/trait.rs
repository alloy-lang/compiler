...
pub(super) fn infer_abstract_member_call(
    ...
    let expected_func_ty = infer_abstract_member_ref(ctx, source_fql.clone(), type_annotation);
    ...
    let mut actual_func_ty = result_ty.clone();
    for arg_ty in arg_types.into_iter().rev() {
        actual_func_ty = MonoType::Function(Box::new(arg_ty), Box::new(actual_func_ty));
    }

    ctx.add_equation(expected_func_ty, actual_func_ty, source_fql.clone());
