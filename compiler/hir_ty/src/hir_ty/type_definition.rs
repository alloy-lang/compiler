use crate::hir_ty::Fql;
use crate::HirTyDatabase;
use alloy_hir as hir;
use alloy_scope::{ScopeIdx, Scopes};
use alloy_workspace::ModuleId;
use itertools::Itertools;

pub fn resolve_path(
    db: &dyn HirTyDatabase,
    current_module_id: ModuleId,
    path: &hir::Path,
    scope: ScopeIdx,
) -> Option<Fql<hir::TypeDefinition>> {
    match path {
        hir::Path::ThisModule(this_path) => {
            get_type_definition_by_name(db, current_module_id, this_path.last(), scope)
        }
        hir::Path::OtherModule(fqn) => {
            let module_slug = fqn.module.iter().map(|n| n.as_str()).join("::");
            let other_module_id = db
                .find_module_by_slug(&*module_slug)
                .expect("somehow, we couldn't find the module");
            get_type_definition_by_name(db, other_module_id, fqn.module.last(), Scopes::ROOT)
        }
        hir::Path::Unknown(_) => None,
    }
}

fn get_type_definition_by_name(
    db: &dyn HirTyDatabase,
    module_id: ModuleId,
    name: &hir::Name,
    scope: ScopeIdx,
) -> Option<Fql<hir::TypeDefinition>> {
    let (hir_module, _) = hir::lower_file(db, module_id);
    let Some((idx, _)) = hir_module.get_type_definition_by_name(name, scope) else {
        return None;
    };

    Some(Fql::new(module_id, idx))
}
